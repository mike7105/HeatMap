...
class Application(ttk.Frame):
    ...
    def draw_dots_thr(self):
        ...
        while not self.queue.empty():
            ...
            with self.lock:
                ...
                self.var_status.set(f"dots: {str(self.var_pb.get() + 1)} from {str(self.df2.shape[0])}")
                self.pgb.step()
    ...
    def open_data(self):
        ...
        if filename:
            ...
            if str.lower(os.path.splitext(filename)[1]) == ".csv":
                ...
                # self.df, _ = pyreadstat.read_sav(filename, usecols=None, apply_value_formats=True)
    ...
    def validate(self) -> bool:
        ...
        :return: Пройдена проверка или нет
        """
        res = ""
    ...
    def change_state(self, is_visible):
        ...
        :param bool is_visible: указывает, на какое состояние поменять, True - активное, False - не активное
        """
